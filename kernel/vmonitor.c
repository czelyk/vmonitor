...
struct vmonitor_dev {
    ...
    /*
     * Protects queue, counters, sequence state,
     * threshold and timer generation state.
     */
    spinlock_t lock;
    ...
    /*
     * Next value generated by the timer.
     */
...
static void generate_automatic_sample_locked(struct vmonitor_dev *dev,
                                             u64 timestamp_ns)
{
    struct vmonitor_sample sample;
    bool pushed;
    ...
    /*
     * Driver owns all fields of automatically generated samples.
     */
    sample.seq = ++dev->seq_counter;
    sample.timestamp_ns = timestamp_ns;
    sample.value_mC = dev->generated_value_mC;
    sample.alarm =
        (sample.value_mC >= dev->threshold_mC) ? 1 : 0;

    /*
     * produced_total counts every sample produced,
     * including samples later dropped because the queue is full.
     */
    dev->produced_total++;
...
static void vmonitor_timer_callback(struct timer_list *timer)
{
    ...
    /*
     * ktime_get_ns() uses a monotonic kernel clock.
     */
    timestamp_ns = ktime_get_ns();

    /*
     * Timer callback cannot sleep.
     * Protect shared state using the existing spinlock.
     */
    spin_lock_irqsave(&dev->lock, flags);

    generate_automatic_sample_locked(dev, timestamp_ns);

    /*
     * Snapshot period while protected.
     * A later sysfs task may modify this field.
     */
 ...
 * read()
 *
 * Exactly one struct vmonitor_sample must be requested.
 *
 * The sample is first peeked, then copied to userspace.
 * It is removed from the queue only if copy_to_user()
 * succeeds.
 */
                             ...
                             loff_t *ppos)
{
    ...
    /*
     * Single-open does not prevent the same fd from being
     * shared by multiple threads/processes.
     */
    ...
    if (!available) {
        /*
         * Blocking read/wait queue is implemented later.
         */
        ret = 0;
        goto out_unlock;
    }
    ...
    /*
     * Never hold a spinlock during copy_to_user().
     */
    if (copy_to_user(buf, &sample, sizeof(sample))) {
        /*
         * Sample stays in the queue.
         */
        ret = -EFAULT;
        goto out_unlock;
    }
    ...
    /*
     * Commit removal only after userspace received
 ...
 *
 * Userspace passes exactly one struct vmonitor_sample.
 * Only value_mC is accepted from userspace.
 *
 * seq, timestamp_ns and alarm are assigned by the driver.
 */
                              ...
                              loff_t *ppos)
{
    ...
    /*
     * Driver-owned sequence and alarm.
     */
    sample.seq = ++dev->seq_counter;
    sample.alarm =
        (sample.value_mC >= dev->threshold_mC) ? 1 : 0;
    ...
    /*
     * A queue-full drop does not make write() itself fail.
     */
...
static const struct file_operations vmonitor_fops = {
    ...
    /*
     * unlocked_ioctl and poll are future tasks.
     */
};
...
static int __init vmonitor_init(void)
{
    ...
    /*
     * Initialize timer before exposing the device.
     */
     ...
     *
     * START/STOP ioctl support will be implemented
     * in a later task.
     */
...
{
    /*
     * Stop timer synchronously before freeing vdev.
     *
     * After del_timer_sync() returns, the callback
